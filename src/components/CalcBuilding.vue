      ...
      <div v-for="cost in data[id].baseCosts" :key="cost.id" class="row g-1">
        ...
        <div class="col-auto">
          <small
            ...
            :class="{
              'text-light':
                costTo(bracket, cost.count, data[id].count, cost.id) <=
                data[cost.id].storage,
              'text-excess':
                costTo(bracket, cost.count, data[id].count, cost.id) <=
                  getStorageCap(cost.id) &&
                costTo(bracket, cost.count, data[id].count, cost.id) >
                  data[cost.id].storage,
              'text-danger':
                costTo(bracket, cost.count, data[id].count, cost.id) >
        ...
        <div class="col-auto text-end" style="width: 75px">
          ...
          <small
            v-if="
              costTo(bracket, cost.count, data[id].count, cost.id) >
                data[cost.id].count &&
              timerTo(bracket, cost.count, data[id].count, cost.id) >
                3600 * 24 * 2
            "
            class="text-timer"
            >{{ $t("bigTimer") }}</small
          ...
          <small
            v-if="
              costTo(bracket, cost.count, data[id].count, cost.id) >
                data[cost.id].count &&
              timerTo(bracket, cost.count, data[id].count, cost.id) > 0 &&
              timerTo(bracket, cost.count, data[id].count, cost.id) <=
                3600 * 24 * 2
            "
            class="text-timer"
            role="timer"
            ><timer
              :value="timerTo(bracket, cost.count, data[id].count, cost.id)"
          /></small>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    timerTo(n, base, current, id) {
      ...
      return this.data[id].prod > 0
        ? (cost - this.data[id].count) / this.data[id].prod
        : 0;
    },
  },
};
